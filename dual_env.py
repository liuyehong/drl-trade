...
class QtradeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(1, self.window, 4), dtype=np.float16)
    ...
    def step(self, action):
        ...
        if self.cash > 0 and order_price_b > self.close[self.t]:
            ...
            print(self.i, self.steps, self.t, self.list_dual_asset[self.t] / self.close0, self.list_asset[self.t] / self.asset0, action,
                  self.list_asset[self.t] / self.asset0 - self.list_dual_asset[self.t] / self.close0)
            ...
            print(self.i, self.steps, self.t, self.list_dual_asset[self.t] / self.close0, self.list_asset[self.t] / self.asset0,
                  action,
                  self.list_asset[self.t] / self.asset0 - self.list_dual_asset[self.t] / self.close0)
        ...
        reward = (self.list_asset[self.t + 1] - self.list_asset[self.t]) / self.list_asset[self.t] - \
                 (self.list_dual_asset[self.t + 1] - self.list_dual_asset[self.t]) / self.list_dual_asset[self.t]
    ...
    def reset(self):
        ...
        if self.i % 2 == 1:
            ...
            self.t0 = self.t
            print(self.t0)
            self.list_dual_asset = self.close
            print(np.average(self.list_asset))
        else:
            print(self.df_dir)
            self.t = self.t0
            print(self.t0)
            self.list_dual_asset = self.list_asset.copy()
            print(np.average(self.list_asset))
